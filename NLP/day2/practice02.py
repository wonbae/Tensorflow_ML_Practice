...
def train(corpus):
    ...
    base = {pos:sum(words.values())for pos, words in pos2words.item()} # P(y) for every y (count for each tag)
...
class HMM_tagger(object):
    ...
    def sent_log_prob(self, sent):
        ...
        #sent->(word,pos) --> p(word,pos) get emission prob. for each (w, t), otherwise unk value
        ...
        log_prob+= sum(
            (self.trans.get(bigram, self.unk) for bigram in bigrams)
        )# get transition prob.
